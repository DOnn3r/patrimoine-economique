...
app.post('/patrimoine/range', (req, res) => {
  ...
  possessions.forEach(p => {
    if (!p.dateFin) { // Considérer seulement les possessions encore actives
      const dateDebutPossession = new Date(p.dateDebut);
      const dateFinPossession = p.dateFin ? new Date(p.dateFin) : new Date();
      
      if (dateDebutPossession <= endDate && dateFinPossession >= startDate) {
        // Calculer la valeur de la possession en fonction de la période
        result.push(p.valeur);
      }
    }
  });
